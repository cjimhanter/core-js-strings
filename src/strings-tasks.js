...
function sumOfCodes(str) {
  return str === null || str === undefined
    ? 0
    : str.split('').reduce((sum, current) => sum + current.charCodeAt(0), 0);
}
...
function countVowels(str) {
  const vovels = str.match(/[aeiouy]/gi);
...
function isPalindrome(str) {
  const letters = str.match(/[qwertyuiopasdfghjklzxcvbnm]/gi);

  return (
    letters.join('').toLowerCase() === letters.reverse().join('').toLowerCase()
  );
}
...
function getCardId(value) {
  ...
  return [...value].length === 3
    ? 9 + suit[value[2]]
    : rank[value[0].charCodeAt(0)] + suit[value[1]];
}
